...
LINUX_PATHOLOGY_FASTQ_FOLDER = replace_env(config_dict['LINUX_PATHOLOGY_FASTQ_FOLDER_NGS21'])
...
if __name__ == "__main__":
    ...
    for index, row in xldf.iterrows():
        ...
        assay_dir = xldf.at[index, "ASSAY_DIR"]
        #print(sample_path)
        #print(status)
        #print(list(status))
        ...
        if sample_path == '' or assay_dir is None or assay_dir == '':
            populate_error_msg(row, "ERROR: Invalid directory or Assay directory", xldf, conn)
            # xldf.at[index, "STATUS"] = "FAILED"
        ...
        assay_folder = assay_dir.split('-')[0]
        ...
        if status == 'RUN' or status == 'RE-RUN':
            ...
            LINUX_HPC_RUN_DIR_CRONJOB_OUT_ASSAY_DIR = LINUX_HPC_ANALYSIS_FOLDER+"/"+ HPC_JOB_PREFIX + "_out_assay_dir.txt"
            ...
            cron_fw.close()
            # Store the output assay folder to copy the output once processed.
            cron_fw = open(LINUX_HPC_RUN_DIR_CRONJOB_OUT_ASSAY_DIR,'w')
            cron_fw.write(assay_folder+"/"+assay_dir)
            cron_fw.close()
            #print(HPC_RUN_CMD)
