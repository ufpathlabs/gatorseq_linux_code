...
def save_workbook():
    try:
        wb.save(GATOR_SEQ_SAMPLE_INPUT_FILE)
    except Exception:
        sys.exit()
...
for row in range(2, sheet.max_row + 1):
    ...
    sample_path=sheet[sample_path_index].value
    status=sheet[status_index].value
    #print(sample_path)
    #print(status)
    #print(list(status))
    ...
    try:
        ...
        sheet[error_index] = "ERROR: Empty directory"
        save_workbook()
        continue

    if sample_path == '':
        sheet[status_index] = "FAILED"
        sheet[error_index] = "ERROR: Invalid directory"
        save_workbook()
        continue
    ...
    if len(directories) == 0:
        sheet[status_index] = "FAILED"
        sheet[error_index] = "ERROR: Following directory extention not found:" + sample_path
        save_workbook()
        continue
    elif len(directories) > 1:
        sheet[status_index] = "FAILED"
        sheet[error_index] = "ERROR: More than one directory extention found:" + ';'.join(directories)
        save_workbook()
        continue
    else:
    ...
    if len(directories) == 0:
        ...
        sheet[error_index] = "WARNING: Directory is not yet uploaded to HPC:" + linux_hpc_sample_path
        save_workbook()
        continue
    elif len(directories) > 1:
        sheet[status_index] = "FAILED"
    ...
    if status == 'RUN':
        ...
        sheet[status_index] = "SUBMITTED"
        sheet[time_stamp_index] = time_stamp 
